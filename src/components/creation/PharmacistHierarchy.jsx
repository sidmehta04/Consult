...
const PharmacistHierarchyManagement = ({ currentUser }) => {
  ...
  const saveHierarchy = async () => {
    try {
      ...
      // Extract pharmacist IDs in order
      const pharmacistHierarchy = assignedPharmacists.sort((a, b) => a.position - b.position).map(pharm => pharm.id);
      
      const roRef = doc(firestore, "users", currentUser.uid);
      
      // Update RO's pharmacist hierarchy and assignToAnyPharmacist setting
      await setDoc(roRef, { 
        pharmacistHierarchy,
        assignToAnyPharmacist 
      }, { merge: true });
      
      // Update all clinics under this RO to assign these pharmacists
      ...
      clinicsSnapshot.forEach((clinic) => {
        // Create pharmacist assignments object for each clinic
        ...
        // Add assignToAnyPharmacist setting to clinic
