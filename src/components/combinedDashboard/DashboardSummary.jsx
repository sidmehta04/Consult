...
const DashboardSummary = ({ doctors, doctorPharmacist, casesData = [] }) => {
  // Calculate summary metrics
  const availableDoctors = doctors.filter(d => d.availabilityStatus === "available").length;
  const busyDoctors = doctors.filter(d => d.availabilityStatus === "busy").length;
  const totalCasesToday = doctors.reduce((sum, d) => sum + (d.todayCases || 0), 0);
  const completedCases = doctors.reduce((sum, d) => sum + (d.completedCases || 0), 0);
  ...
  // Check if unavailable doctors/pharmacists have pending cases using actual cases data
  const unavailableWithCases = unavailableDoctors.filter(doctor => {
    if (doctorPharmacist === "doctor") {
      // Check if doctor has pending cases - same logic as analytics dashboard
      return casesData.some(item => 
        item.assignedDoctors?.primary === doctor.id && item.doctorCompleted === false
      );
    } else {
      // Check if pharmacist has pending cases - same logic as analytics dashboard
      return casesData.some(item => 
  ...
  const unavailableWithoutCasesCount = unavailableCount - unavailableWithCasesCount;
