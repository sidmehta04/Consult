  ...
  return (
    <div className="bg-white shadow-md rounded-lg overflow-hidden">
      <div className={`px-4 py-2 ${
        doctor.availabilityStatus === "available" ? "bg-green-50" :
        doctor.availabilityStatus === "busy" ? "bg-yellow-50" :
        doctor.availabilityStatus === "on_break" ? "bg-blue-50" : "bg-red-50"
      }`}>
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold">{doctor.name}</h3>
          <span className={`px-2 py-1 text-xs rounded-full font-semibold ${getStatusBadgeColor(doctor.availabilityStatus)}`}>
            {formatStatus(doctor.availabilityStatus)}
          </span>
        </div>
        <div className="text-sm text-gray-600">
          {doctor.empId && `ID: ${doctor.empId}`} • {formatShiftTiming(doctor.shiftTiming)}
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="p-2 border border-gray-100 rounded">
            ...
            <div className="text-lg font-medium text-green-600">{doctor.completedCases}</div>
          </div>
          <div className="p-2 border border-gray-100 rounded">
            <div className="text-xs text-gray-500">Pending</div>
            <div className="text-lg font-medium text-yellow-600">{doctor.pendingCases}</div>
          </div>
          <div className="p-2 border border-gray-100 rounded">
            <div className="text-xs text-gray-500">Incomplete</div>
            <div className="text-lg font-medium text-red-600">{doctor.incompleteCases}</div>
          </div>
          <div className="p-2 border border-gray-100 rounded">
            <div className="text-xs text-gray-500">Avg. TAT</div>
            <div className="text-lg font-medium">{doctor.averageTAT} <span className="text-xs">min</span></div>
          </div>
        </div>
