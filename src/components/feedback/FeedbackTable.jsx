  ...
  useEffect(() => {
    ...
    if (currentUser) {
      ...
      if (currentUser.email === "Akash.das@m-insure.in") {
        ...
        // Regular QA users see tickets where they are assigned (either in qaEmails array or old qaEmail field)
        ticketsQuery = query(
          collection(firestore, "tickets"),
          orderBy("createdAt", "desc")
        );
      ...
      const unsubscribe = onSnapshot(ticketsQuery, (snapshot) => {
        ...
        if (currentUser.email !== "Akash.das@m-insure.in") {
          fetchedTickets = fetchedTickets.filter(ticket => {
            ...
            return (ticket.qaEmails && ticket.qaEmails.includes(currentUser.email)) ||
                   ticket.qaEmail === currentUser.email;
          });
        }
